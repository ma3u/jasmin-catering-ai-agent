#!/usr/bin/env python3
"""
Configure OpenAI Assistant using standard OpenAI API (not Azure)
This script assumes the assistant exists on platform.openai.com
"""
...
# Check for OpenAI API key (different from Azure)
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
if not OPENAI_API_KEY:
    print("❌ Error: OPENAI_API_KEY not found in environment")
    print("\nTo use this script, you need:")
    print("1. An OpenAI API key from platform.openai.com")
    print("2. Add to .env: OPENAI_API_KEY=sk-...")
    sys.exit(1)

# OpenAI client (not Azure)
...
def check_assistant_exists() -> bool:
    ...
    try:
        ...
        print(f"❌ Assistant not found: {str(e)}")
        return False
...
def main():
    ...
    print("🚀 OpenAI Assistant Configuration")
    print("=================================")
    print(f"Assistant ID: {ASSISTANT_ID}")
    print("Platform: OpenAI (platform.openai.com)")
    print()
    
    # Check if assistant exists
    if not check_assistant_exists():
        print("\n⚠️ This assistant doesn't exist on your OpenAI account.")
        print("Possible reasons:")
        print("1. The assistant was created by someone else")
        print("2. You're using the wrong API key")
        print("3. The assistant ID is incorrect")
        sys.exit(1)
