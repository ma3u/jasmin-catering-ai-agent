...
# Deploy LogicApp with Google OAuth Credentials (SECURE VERSION)
# Uses credentials from .secret/ directory (never committed to GitHub)

set -e

echo "🚀 Deploying LogicApp with Google OAuth credentials..."
echo ""
...
if [ ! -f "$CREDS_FILE" ]; then
    ...
    echo "   mkdir -p .secret"
    echo "   # Add your Google OAuth credentials to .secret/google-oauth-credentials.json"
    exit 1
fi

# Verify .gitignore protection
if ! grep -q ".secret/" .gitignore; then
    echo "⚠️  Warning: .secret/ not in .gitignore - updating..."
...
echo "💬 Slack: #gmail-inbox (mabured.slack.com)"
echo ""
...
az resource create \
  ...
  --name gmail-oauth-jasmin \
  --location westeurope \
  --properties '{
    "displayName": "Gmail - OAuth - mabu.mate@gmail.com", 
...
echo "📝 Creating LogicApp workflow..."

# Create workflow definition with proper connections
cat > logicapp/gmail-oauth-workflow.json << EOF
{
  ...
  "parameters": {
    "\$connections": {
      "defaultValue": {
        "gmail": {
          "connectionId": "/subscriptions/b58b1820-35f0-4271-99be-7c84d4dd40f3/resourceGroups/$RESOURCE_GROUP/providers/Microsoft.Web/connections/gmail-oauth-jasmin",
          "connectionName": "gmail-oauth-jasmin",
          "id": "/subscriptions/b58b1820-35f0-4271-99be-7c84d4dd40f3/providers/Microsoft.Web/locations/westeurope/managedApis/gmail"
        }
      },
      "type": "Object"
...
az resource update \
  ...
  --name $LOGIC_APP_NAME \
  --set properties.definition="$(cat logicapp/gmail-oauth-workflow.json)"

echo ""
echo "✅ LogicApp deployed successfully!"
echo ""
echo "🔧 FINAL STEP: Configure Gmail Authentication in Azure Portal"
echo ""
...
echo "   - Select 'Change connection' → 'Add new'"
...
echo "   📧 Gmail → LogicApp → HTTPS → Slack #gmail-inbox ✅"
echo ""
echo "🎉 No restrictions! Fully Microsoft-compliant solution!"

# Don't backup secrets to GitHub
echo ""
echo "🔒 Security Note: Credentials are safely stored in .secret/ and will never be committed to GitHub"
